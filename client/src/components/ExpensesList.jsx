...
const ExpenseList = () => {
  ...
  const handleSaveToDatabase = async () => {
    ...
    await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/api/expenses/set`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user, expenses: expenseInputs }),
    });

    const response = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/api/expenses/setFlag`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
